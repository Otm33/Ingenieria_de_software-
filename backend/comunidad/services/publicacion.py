...
class PublicacionService:
    ...
    def actualizar_estado_publicacion(self, usuario, publicacion_id, esta_activa):
        ...
        updated_count = Publicacion.objects.filter(id=publicacion_id, usuario=usuario).update(esta_activa=esta_activa)
        print(f"DEBUG: Updated publication {publicacion_id} to esta_activa={esta_activa}, rows affected: {updated_count}")
        print(f"DEBUG: Publication {publicacion_id} current state after update: {list(Publicacion.objects.filter(id=publicacion_id).values('esta_activa'))}")
        logger.info(f"Updated publication {publicacion_id} to esta_activa={esta_activa}, rows affected: {updated_count}")
        logger.info(f"Publication {publicacion_id} current state after update: {list(Publicacion.objects.filter(id=publicacion_id).values('esta_activa'))}")
