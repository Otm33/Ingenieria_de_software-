...
const comercioController = inject('comercioController')
...
const cargarDatos = async () => {
  ...
  try {
    const [catalogo, saldo, sesion] = await Promise.all([
      ...
      comercioController.obtenerMiSaldoComercial(),
    ...
    if (esComercio.value) {
      const listaClientes = await comercioController.obtenerClientes()
...
const emitirVuelto = async () => {
  ...
  try {
    const respuesta = await comercioController.emitirVuelto(
...
const pagarConSaldo = async () => {
  ...
  try {
    const respuesta = await comercioController.pagarConSaldo(formPago.comercioId, formPago.monto)
    resultadoPago.value = {
